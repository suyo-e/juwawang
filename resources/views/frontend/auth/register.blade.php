...
<div class="container">
    ...
    <div class="register" id="step_1">
        ...
        <div class="code">
            <input class="num inpt" placeholder=" 请输入验证码" type="number" name="verify_code">
        ...
        <div class="Aradio">
            <label><input class="Fruit" type="radio" value="3" name="type" checked="">用户</label>
        ...
        <label class="imy">
            <input type="checkbox" id="readProtocol" name="checked">我已阅读并同意
            <span>《服务协议》</span>
        </label>
    </div>
    <div id="step_2" class="register" style="display:none">
        ...
        <select class="inputAll" name="manufa_2" id="manufa-2">
            <option value="0">请选择二级行业</option>
        </select>
        <input class="inputAll" id="city-picker" value="{{ old('province_city') }}" placeholder="请选择省份城市" />
        ...
        $.get('/categories?type=' + type, function(data) {
            ...
            $("#manufa-1").change(function() {
                ...
                $.get('/categories?parent_id=' + parent_id, function(data) {
                    ...
                    if(data.length == 0) {
                        $("#manufa-2").append('<option>暂无二级行业</option>');
                    }
                    else {
